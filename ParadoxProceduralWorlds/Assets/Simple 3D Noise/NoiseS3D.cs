...
public static class NoiseS3D {
	...
	public static RenderTexture GetNoiseRenderTexture(int width, int height, SimplexNoiseSettings InNoiseSettings, bool normalize = true)
	{
		RenderTexture retTex = new RenderTexture(width, height, 0);
		retTex.enableRandomWrite = true;
		retTex.Create();
		...
		SetShaderVars(shader, 
			new Vector2(InNoiseSettings.m_offset.x, InNoiseSettings.m_offset.y), 
			normalize, 
			InNoiseSettings.m_frequency,
			InNoiseSettings.m_gain,
			InNoiseSettings.m_lacunarity,
			InNoiseSettings.m_waterLevel,
			kernel
			);
		shader.SetInts("reses", resInts);
