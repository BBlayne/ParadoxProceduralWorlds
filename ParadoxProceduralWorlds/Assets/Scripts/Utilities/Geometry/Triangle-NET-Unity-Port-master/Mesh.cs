    ...
    public class Mesh : IMesh
    {
        ...
        internal void TransferNodes(IList<Vertex> points)
        {
            ...
            foreach (var p in points)
            {
                ...
                // otherwise throws an error, not sure
                // why, when trying to triangulate the
                // same set of points with two different
                // polygons.
                if (!this.vertices.ContainsKey(p.hash))
                {
                    this.vertices.Add(p.hash, p);
                }
                else
                {
                    UnityEngine.Debug.Log("Something went wrong with p: " + p.ID);
                }
